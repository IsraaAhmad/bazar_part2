...
def update_price(id):
    ...
    requests.put("http://192.168.1.30:4000/update_price_s/"+str(id),{'price':price})
...
def increase(id):
    ...
    if len(rows1)== 0:
        return "the book is not exist"
    amount = request.form.get('amount')
    requests.put("http://192.168.1.30:4000/increase_s/"+str(id),{'amount':amount})
...
def decrease(id):
    ...
    if len(rows1)== 0:
        ...
        amount = request.form.get('amount')
        requests.put("http://192.168.1.30:4000/decrease_s/"+str(id),{'amount':amount})
